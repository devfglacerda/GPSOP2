...
// --- MODIFICACAO: REGISTRO DE HISTORICO PARA BOTAO VOLTAR ---
// Grava a URL atual se for uma visualização (GET) válida e logada
if (isset($_SESSION['Aplic']) && 
    !$dialogo && 
    !$sem_cabecalho && 
    $_SERVER['REQUEST_METHOD'] == 'GET' && 
    !isset($_REQUEST['logout'])) { // Não gravar se estiver saindo

    // Inicializa o array se não existir
    if (!isset($_SESSION['gpweb_historico'])) {
        $_SESSION['gpweb_historico'] = array();
    }

    $urlAtual = $_SERVER['REQUEST_URI'];
    
    // Verifica a última URL gravada
    $ultimaUrl = empty($_SESSION['gpweb_historico']) ? '' : end($_SESSION['gpweb_historico']);

    // Só adiciona se a URL for diferente da última (evita duplicar F5)
    if ($ultimaUrl !== $urlAtual) {
        $_SESSION['gpweb_historico'][] = $urlAtual;

        // Mantém apenas os últimos 10 passos para economizar memória
        if (count($_SESSION['gpweb_historico']) > 10) {
            array_shift($_SESSION['gpweb_historico']);
        }
    }
}
// --- FIM DA MODIFICACAO ---

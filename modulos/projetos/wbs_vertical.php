...
if ($projeto_id){
    ...
    elseif (!$config['termo_abertura_obrigatorio'] && $Aplic->checarModulo('tarefas', 'adicionar') && $Aplic->checarModulo('projetos', 'adicionar')){
        ...
        echo '<tr><td colspan=2 style="background-color: #2a343e" width="100%">';
        require_once BASE_DIR.'/lib/coolcss/CoolControls/CoolMenu/coolmenu.php';
        $km = new CoolMenu("km");
        $km->scriptFolder ='lib/coolcss/CoolControls/CoolMenu';
        $km->styleFolder="default";
...
function popProjeto() {
    ...
    else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setProjeto&tabela=projetos&usuario_id='+document.getElementById('usuario_id').value+'&cia_id='+document.getElementById('cia_id').value, 'Projetos','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
    }

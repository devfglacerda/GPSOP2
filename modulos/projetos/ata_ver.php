...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Número', 'Número desta entegra').'Número:'.dicaF().'</td><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->ata_cor.'"><font color="'.melhorCor($obj->ata_cor).'"><b>'.($obj->ata_numero<100 ? '0' : '').($obj->ata_numero<10 ? '0' : '').$obj->ata_numero.'<b></font></td></tr>';
if ($obj->ata_responsavel) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável', 'Responsável por executar a mudança.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->ata_responsavel, '','','esquerda').'</td></tr>';		
...
if ($saida_quem) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Equipe', 'Quais '.$config['usuarios'].' estarão na equipe desta ata de reunião.').'Equipe:'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_quem.'</td></tr>';
if ($obj->ata_data_inicio) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data', 'A data desta ata de reunião').'Data:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->ata_data_inicio).($obj->ata_data_fim ? ' - '.substr($obj->ata_data_fim, 11, 5) : '').'</td></tr>';
if ($obj->ata_local) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Local', 'O local da reunião.').'Local:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->ata_local.'</td></tr>';
if ($obj->ata_relato) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Relato', 'Deliberar sobre a aprovação da mudança.').'Relato:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->ata_relato.'</td></tr>';
...
if ($saida) echo '<tr><td align="right"  style="white-space: nowrap">'.dica('Ações', 'As ações a serem desenvolvidas.').'Ações:'.dicaF().'</td><td>'.$saida.'</td></tr>';



if ($obj->ata_proxima_data_inicio) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Próxima Reunião', 'A data da próxima reunião').'Próxima reunião:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->ata_proxima_data_inicio).($obj->ata_proxima_data_fim ? ' - '.substr($obj->ata_proxima_data_fim, 11, 5) : '').'</td></tr>';
if ($obj->ata_proxima_local) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Local da Próxima', 'O local da próxima reunião.').'Local da próxima:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->ata_proxima_local.'</td></tr>';
...
if ($pautas && count($pautas)) $saida.='</table>';
if ($saida) echo '<tr><td align="right"  style="white-space: nowrap">'.dica('Pauta da Próxima', 'A pauta da próxima reunião.').'Pauta da próxima:'.dicaF().'</td><td>'.$saida.'</td></tr>';

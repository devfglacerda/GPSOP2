...
$projeto_tipo = getSisValor('TipoProjeto');
$sp_obj = new CProjeto();
...
echo '<table style="width:100%; border-collapse:collapse; margin:0px 0; font-size:16px;">';
echo '<tr><td>';
echo '<table style="width:100%; border-collapse:collapse; background:#1a1a1a; border-radius:8px; overflow:hidden;">';
echo '<tr>';
echo '<th style="width:40px; padding:10px; background:#333; color:#fff; text-align:center; font-weight:bold; border-right:1px solid #555;"> </th>';
echo '<th style="padding:10px; background:#333; color:#fff; text-align:center; font-weight:bold; border-right:1px solid #555;">Projeto</th>';
echo '<th style="padding:10px; background:#333; color:#fff; text-align:center; font-weight:bold; border-right:1px solid #555;">Fiscal</th>';
echo '<th style="padding:10px; background:#333; color:#fff; text-align:center; font-weight:bold;">Construtora</th>';
...
//echo '<th>'.dica('Construtora', 'Clique para ordenar '.$config['genero_projeto'].'s '.$config['projetos'].' pel'.$config['genero_usuario'].'s '.$config['usuarios'].' responsáveis pelos mesmos').'Construtora'.dicaF().'</th>';
//echo '<th>'.dica('Status da Obr', 'Clique para ordenar '.$config['genero_projeto'].'s '.$config['projetos'].' pel'.$config['genero_usuario'].'s '.$config['usuarios'].' responsáveis pelos mesmos').'Status da Obra'.dicaF().'</th>';
...
foreach ($st_projetos_arr as $projeto) {
	...
	if ($linha['projeto_id']) {
		...
		$s .= '<td '.$linha_class.' align="center" style="padding:10px; font-size:15px; color:#333; background:#f9f9f9; border-bottom:1px solid #ddd;">'
    . link_usuario($s_projeto->projeto_supervisor) .
    '</td>';
...
//$s .= '<td '.$linha_class.' align="center">'.prioridade($s_projeto->projeto_prioridade, true).'</td>';
		...
		//$s .= '<td '.$linha_class.' align="center">'.sprintf('%.1f%%', $s_projeto->projeto_percentagem).'</td>';
		//$s .= '<td '.$linha_class.' align="center">'.$projetoStatus[$s_projeto->projeto_status].'</td>';//

		//$s .= '<td '.$linha_class.' align="center">'.$projeto_tipo[$s_projeto->projeto_tipo].'</td>	</tr>';
		}
	}
echo $s;
/*

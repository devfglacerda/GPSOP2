<?php
global $envolvimento, $selecao, $chamarVolta, $selecionado, $edicao, $a, $filtro_prioridade_demanda, $estilo_interface, $dialogo, $tab, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $favorito_id, $podeEditar,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
	$tema_id,
...
if ($tab==0 && $a=='demanda_lista') $sql->adOnde('demanda_caracteristica_projeto IS NULL OR demanda_caracteristica_projeto=0');
elseif ($tab==1 && $a=='demanda_lista') $sql->adOnde('demanda_caracteristica_projeto=1');
elseif ($tab==2 && $a=='demanda_lista') $sql->adOnde('demanda_caracteristica_projeto=-1');
if ($tab< 3 && $a=='demanda_lista') $sql->adOnde('demanda_projeto IS NULL OR demanda_projeto=0');
elseif ($tab==3 && $a=='demanda_lista') $sql->adOnde('demanda_projeto IS NOT NULL AND demanda_projeto!=0');

if ($tab<5 || $a!='demanda_lista') $sql->adOnde('demanda_ativa=1');
...
if ($filtro_prioridade_demanda) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';

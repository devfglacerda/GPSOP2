...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Número', 'Número desta ata de reunião').'Número:'.dicaF().'</td><td width="100%" colspan="2">'.($obj->ata_numero<100 ? '0' : '').($obj->ata_numero<10 ? '0' : '').$obj->ata_numero.'</td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="ata_cor" value="'.($obj->ata_cor ? $obj->ata_cor : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável', 'Toda ata de reunião deve ter um responsável.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="ata_responsavel" name="ata_responsavel" value="'.($obj->ata_responsavel ? $obj->ata_responsavel : $Aplic->usuario_id).'" /><input type="text" id="nome_responsavel" name="nome_responsavel" value="'.nome_om(($obj->ata_responsavel ? $obj->ata_responsavel : $Aplic->usuario_id),$Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popResponsavel();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['tarefa']).' Relacionada', 'Caso a ata de reunião seja específica de um'.($config['genero_tarefa']=='a' ?  'a' : '').' '.$config['tarefa'].' d'.$config['genero_projeto'].' '.$config['projeto'].', este campo deverá constar o nome d'.$config['genero_tarefa'].' '.$config['tarefa'].'.').ucfirst($config['tarefa']).':'.dicaF().'</td><td align="left" valign="top" style="white-space: nowrap"><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="ata_tarefa" value="'.$obj->ata_tarefa.'" /><input type="text" id="tarefa_nome" name="tarefa_nome" value="'.nome_tarefa($obj->ata_tarefa).'" style="width:450px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popTarefa();">'.imagem('icones/tarefa_p.gif','Selecionar '.ucfirst($config['tarefa']),'Clique neste ícone '.imagem('icones/tarefa_p.gif').' escolher à qual '.$config['tarefa'].' a entrega irá pertencer.<br><br>Caso não escolha um'.($config['genero_tarefa']=='a' ?  'a' : '').' '.$config['tarefa'].', a entrega será d'.$config['genero_projeto'].' '.$config['projeto'].' tod'.$config['genero_projeto'].'.').'</a></td></tr></table></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nível de Acesso', 'A ata de reunião pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar a ata de reunião.</li><li><b>Protegido I</b> - Todos podem ver, porem apenas o responsável e os designados para a ata de reunião podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Protegido III</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante I</b> - Somente o responsável e os designados para a ata de reunião ver e editar a ata de reunião</li><li><b>Participantes II</b> - Somente o responsável e os designados podem ver e apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Participantes III</b> - Somente o responsável e os designados para a ata de reunião podem ver a mesma, e o responsável editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($ata_acesso, 'ata_acesso', 'class="texto"', ($ata_id ? $obj->ata_acesso : $config['nivel_acesso_padrao'])).'</td></tr>';
...
if ($saida_quem) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão executando esta ata de reunião.').'Quem:'.dicaF().'</td><td width="100%" colspan="2">'.$saida_quem.'</td></td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nova Reunião', 'Marque esta opção se há previsão de uma nova reunião.').'Nova Reunião:'.dicaF().'</td><td colspan="2"><input type="checkbox" name="tem_proxima" id="tem_proxima" '.($obj->ata_proxima_data_inicio  ? 'checked="checked"' : '').' value="1" onchange="mostrar_proxima();" /></td></tr>';
...
echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre a '.($ata_id > 0 ? 'modificação' : 'criação').' da ata de reunião.').'Notificar:'.dicaF().'</td>';
echo '<td>';

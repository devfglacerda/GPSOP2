...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('VerGUTTab', getParam($_REQUEST, 'tab', null));
$tab = $Aplic->getEstado('VerGUTTab') !== null ? $Aplic->getEstado('VerGUTTab') : 0;

$gut_id = intval(getParam($_REQUEST, 'gut_id', null));
...
if (!$dialogo){
	...
	$bloquear=($obj->gut_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);
...
if ($obj->gut_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por esta matriz GUT.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_secao($obj->gut_dept).'</td></tr>';
$saida_depts='';

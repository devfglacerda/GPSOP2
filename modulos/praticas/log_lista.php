...
if (!$dialogo){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	//$novo_risco=($Aplic->checarModulo('praticas', 'adicionar', null, 'risco_respostas') ? '<tr><td style="white-space: nowrap">'.dica('Novo '.ucfirst($config['risco_resposta']), 'Criar '.($config['genero_risco_resposta']=='o' ? 'um' : 'uma').' novo '.$config['risco_resposta'].'.').'<a href="javascript: void(0)" onclick="javascript:env.a.value=\'risco_resposta_pro_editar\'; env.submit();" ><img src="'.acharImagem('risco_resposta_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
	
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de registros de ocorrência.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&tab='.$tab.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

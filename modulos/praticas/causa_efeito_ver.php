...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('VerCausaEfeitoTab', getParam($_REQUEST, 'tab', null));
$tab = $Aplic->getEstado('VerCausaEfeitoTab') !== null ? $Aplic->getEstado('VerCausaEfeitoTab') : 0;

$causa_efeito_id = intval(getParam($_REQUEST, 'causa_efeito_id', null));
...
if (!$dialogo){
	...
	$bloquear=($obj->causa_efeito_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);
...
if ($obj->causa_efeito_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por esta causa-efeito.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_secao($obj->causa_efeito_dept).'</td></tr>';
$saida_depts='';

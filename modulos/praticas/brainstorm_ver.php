...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('VerBraistormTab', getParam($_REQUEST, 'tab', null));
$tab = $Aplic->getEstado('VerBraistormTab') !== null ? $Aplic->getEstado('VerBraistormTab') : 0;

$brainstorm_id = intval(getParam($_REQUEST, 'brainstorm_id', null));
...
if (!$dialogo){
	...
	$bloquear=($obj->brainstorm_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);
...
if ($obj->brainstorm_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por este brainstorm.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_secao($obj->brainstorm_dept).'</td></tr>';
$saida_depts='';

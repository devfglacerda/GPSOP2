...
global $envolvimento, $selecao, $chamarVolta, $selecionado, $edicao, $estilo_interface, $sql, $perms, $Aplic, $cia_id, $lista_cias, $pesquisar_texto, $usuario_id, $dept_id, $lista_depts, $tab, $ordem, $ordenar, $favorito_id, $dialogo, $filtro_prioridade_checklist,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
...
if ($tab==0 || $a!='checklist_lista') $sql->adOnde('checklist_ativo=1');
...
if ($exibir['checklist_cor']) echo '<th width=16><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=checklist_cor&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='checklist_cor' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cor do Checklist', 'Neste campo fica a cor de identificação do checklist.').'Cor'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=checklist_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='checklist_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome do Checklist', 'Neste campo fica um nome para identificação do checklist.').'Nome'.dicaF().'</a></th>';

if ($exibir['checklist_aprovado'] && $Aplic->profissional) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=checklist_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='checklist_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_checklist) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';

if ($exibir['checklist_descricao']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=checklist_descricao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='checklist_descricao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Descrição do Checklist', 'Neste campo fica a descrição do checklist.').'Descrição'.dicaF().'</a></th>';
...
if ($exibir['checklist_dept']) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=checklist_dept&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica(ucfirst($config['departamento']), strtoupper($config['genero_dept']).' '.strtolower($config['departamento']).' responsável.').($ordenar=='checklist_dept' ? imagem('icones/'.$seta[$ordem]) : '').ucfirst($config['departamento']).dicaF().'</a></th>';
...
if ($exibir['checklist_responsavel']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=checklist_responsavel&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='checklist_responsavel' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'O '.$config['usuario'].' responsável pelo checklist.').'Responsável'.dicaF().'</a></th>';
...
if ($exibir['checklist_quantidade']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=qnt&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='qnt' ? imagem('icones/'.$seta[$ordem]) : '').dica('Quantidade', 'A quantidade de perguntas inseridas no checklist.').'<b>Qnt</b>'.dicaF().'</a></th>';
echo '</tr>';
...
foreach ($checklists as $linha) {
	if (permiteAcessarChecklist($linha['checklist_acesso'],$linha['checklist_id'])){
		...
		if ($exibir['checklist_aprovado'] && $Aplic->profissional) echo '<td width=30 align="center">'.($linha['checklist_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		...
		if ($exibir['checklist_dept']) echo '<td>'.link_secao($linha['checklist_dept']).'</td>';	

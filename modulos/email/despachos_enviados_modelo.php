...
global $estilo_interface, $localidade_tipo_caract, $pesquisar_texto, $tab, $usuario_id, $dialogo;
...
'<th><a class="hdr" href="index.php?m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=modelo_usuario.modelo_id&ordem='.($ordem ? '0' : '1').'">'.($ordenar=='modelo_usuario.modelo_id' ? imagem('icones/'.$seta[$ordem]) : '').dica('Documento', 'Clique para ordenar os despachos pelo documento ao qual o mesmo faz referência.<br><br>Ao clicar em cima da data será mostrado o texto do despacho').'Documento'.dicaF().'</a></th></tr>';

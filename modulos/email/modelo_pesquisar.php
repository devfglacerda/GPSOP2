...
if (!$dialogo){
	...
	if ($Aplic->profissional)  {
		...
		if ($Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso', null, null)) {
			...
			$tipos['popPDCLItem']=ucfirst($config['pdcl_item']);
			}
		}
	asort($tipos);
...
echo '<tr><td align="left" colspan="20" width="100%" id="barra" style="background-color: #212121">';
require_once BASE_DIR.'/lib/coolcss/CoolControls/CoolMenu/coolmenu.php';
$km = new CoolMenu("km");
$km->scriptFolder ='lib/coolcss/CoolControls/CoolMenu';
$km->styleFolder="default";

if ($Aplic->checarModulo('email', 'acesso', $Aplic->usuario_id, 'protocolar_modelo') && !($anexar_documento || $referenciar_documento)) 
	
$km->Add("root",'Menu','Menu');

$km->Add("Menu","protocolar","Protocolar", "javascript: void(0);' onclick='limpar_pesquisa(); env.a.value=\"modelo_protocolar\"; env.item_menu.value=\"a_protocolar\"; env.pagina.value=1; env.submit();");
...
$km->Add("Menu","enviados","Enviados", "javascript: void(0);' onclick='env.item_menu.value=\"enviado\"; env.pesquisar.value=0; env.pagina.value=1; env.submit();");
...
$mod = $sql->Resultado();
$sql->limpar();
$km->Add("Menu","elaboracao","Elaboração".($mod ? ' ('.$mod.')' : ''), "javascript: void(0);' onclick='limpar_pesquisa(); env.modelo_tipo_id.value=0; env.item_menu.value=\"elaboracao\"; env.pesquisar.value=0; env.caixa_elaborar.value=1; env.pagina.value=1; env.submit();");
...
$sql->limpar();
$km->Add("Menu","a_protocolar","à Protocolar".($mod ? ' ('.$mod.')' : ''), "javascript: void(0);' onclick='limpar_pesquisa(); env.item_menu.value=\"a_protocolar\"; env.modelo_tipo_id.value=0; env.pesquisar.value=0; env.caixa_para_protocolar.value=1; env.pagina.value=1; env.submit();");
$km->Add("Menu","protocolados","Protocolados", "javascript: void(0);' onclick='limpar_pesquisa(); env.item_menu.value=\"protocolado\"; env.modelo_tipo_id.value=0; env.pesquisar.value=0; env.caixa_protocolado.value=1; env.pagina.value=1; env.submit();");
$km->Add("Menu","assinados","Assinados", "javascript: void(0);' onclick='limpar_pesquisa(); env.item_menu.value=\"assinado\"; env.modelo_tipo_id.value=0; env.pesquisar.value=0; env.caixa_assinado.value=1; env.pagina.value=1; env.submit();");
...
elseif ($item_menu=='entrada'){
  ...
  $sql->esqUnir('cias', 'cias', 'cia_id = contato_cia');
	...
	$sql->adCampo('modelos.modelo_id, modelo_usuario.modelo_usuario_id, modelo_usuario.tipo, modelos.class_sigilosa, modelo_dados_data, modelo_protocolo, modelo_tipo_nome, modelo_usuario.datahora, modelo_assunto, modelo_usuario.datahora, modelo_usuario.cor, modelo_usuario.nota, contatos.contato_funcao,modelo_assunto, modelo_usuario.status, modelo_usuario.de_id, modelo_usuario.nome_de, modelo_usuario.funcao_de');
...
elseif ($item_menu=='pendente'){
  ...
  $sql->esqUnir('cias', 'cias', 'cia_id = contato_cia');
	$sql->esqUnir('depts', 'depts', 'dept_id = contato_dept');
	$sql->esqUnir('modelo_anotacao', 'modelo_anotacao', 'modelo_anotacao.modelo_anotacao_id = modelo_usuario.modelo_anotacao_id');
  $sql->adCampo('modelos.modelo_id, modelo_usuario.tipo, modelos.class_sigilosa, modelo_tipo_nome, modelo_usuario.modelo_usuario_id, modelo_usuario.datahora, modelo_dados_data, modelo_protocolo, modelo_assunto, modelo_usuario.datahora, modelo_usuario.cor, modelo_usuario.nota, contatos.contato_funcao,modelo_assunto, modelo_usuario.status, modelo_usuario.de_id, modelo_usuario.nome_de, modelo_usuario.funcao_de');
	$sql->adCampo('modelo_anotacao.texto AS texto_nota');
  $sql->adCampo('cia_nome, dept_nome');
	$sql->adCampo(($config['militar'] < 10 ? 'concatenar_tres(contato_posto, \' \', contato_nomeguerra)' : 'contato_nomeguerra').' AS nome_usuario');
...
elseif ($item_menu=='arquivado') {
 ...
 $sql->esqUnir('modelos_dados','modelos_dados','modelos_dados.modelo_dados_modelo = modelos.modelo_id');
  ...
  $sql->esqUnir('cias', 'cias', 'cia_id = contato_cia');
	$sql->esqUnir('depts', 'depts', 'dept_id = contato_dept');
	$sql->esqUnir('modelo_anotacao', 'modelo_anotacao', 'modelo_anotacao.modelo_anotacao_id = modelo_usuario.modelo_anotacao_id');
  $sql->adCampo('modelos.modelo_id, modelo_usuario.tipo, modelos.class_sigilosa, modelo_tipo_nome, modelo_dados_data, modelo_protocolo, modelo_usuario.modelo_usuario_id, modelo_usuario.datahora, modelo_assunto, modelo_usuario.datahora, modelo_usuario.cor, modelo_usuario.nota, contatos.contato_funcao,modelo_assunto, modelo_usuario.status, modelo_usuario.de_id, modelo_usuario.nome_de, modelo_usuario.funcao_de');
	$sql->adCampo('modelo_anotacao.texto AS texto_nota');
  $sql->adCampo('cia_nome, dept_nome');
	$sql->adCampo(($config['militar'] < 10 ? 'concatenar_tres(contato_posto, \' \', contato_nomeguerra)' : 'contato_nomeguerra').' AS nome_usuario');
	$sql->adOnde('para_id= '.$Aplic->usuario_id);
...
elseif ($item_menu=='enviado'){
 ...
 $sql->esqUnir('modelos_dados','modelos_dados','modelos_dados.modelo_dados_modelo = modelos.modelo_id');
  ...
  $sql->esqUnir('cias', 'cias', 'cia_id = contato_cia');
	$sql->esqUnir('depts', 'depts', 'dept_id = contato_dept');
	$sql->esqUnir('modelo_anotacao', 'modelo_anotacao', 'modelo_anotacao.modelo_anotacao_id = modelo_usuario.modelo_anotacao_id');
  $sql->adCampo('modelos.modelo_id, modelo_usuario.tipo, modelo_dados_data, modelo_protocolo, modelos.class_sigilosa, modelo_tipo_nome, modelo_usuario.modelo_usuario_id, modelo_usuario.datahora, modelo_assunto, modelo_usuario.datahora, modelo_usuario.cor, modelo_usuario.nota, contatos.contato_funcao,modelo_assunto, modelo_usuario.status, modelo_usuario.de_id, modelo_usuario.nome_de, modelo_usuario.funcao_de');
	$sql->adCampo('modelo_anotacao.texto AS texto_nota');
  $sql->adCampo('cia_nome, dept_nome');
	$sql->adCampo(($config['militar'] < 10 ? 'concatenar_tres(contato_posto, \' \', contato_nomeguerra)' : 'contato_nomeguerra').' AS nome_usuario');
...
elseif ($item_menu=='a_protocolar'){
  ...
  $sql->esqUnir('modelos_dados', '', 'modelos_dados.modelo_dados_modelo = modelos.modelo_id');
  $sql->adCampo('modelos.modelo_id, MAX(modelo_dados_data), modelo_protocolo, modelos.class_sigilosa, modelos.modelo_data_assinado, modelos.modelo_criador_original, modelos.modelo_assunto, modelos_tipo.modelo_tipo_nome, modelo_data_aprovado');
...
elseif ($item_menu=='protocolado'){
  ...
  $sql->esqUnir('modelos_tipo','modelos_tipo','modelo_tipo = modelo_tipo_id');
  $sql->esqUnir('modelos_dados', 'modelos_dados', 'modelo_dados_modelo=modelos.modelo_id');
  $sql->adCampo('modelos.modelo_id, modelo_tipo_nome, modelo_protocolo, modelo_assunto, modelo_assunto, modelo_data_protocolo, modelos.class_sigilosa');
	$sql->adOnde('modelos_dados_criador	'.($coletivo ? 'IN ('.$Aplic->usuario_lista_grupo.')' : '='.(int)$Aplic->usuario_id));
...
elseif ($item_menu=='assinado'){
  ...
  $sql->esqUnir('modelos_tipo','modelos_tipo','modelo_tipo = modelo_tipo_id');
  $sql->esqUnir('modelos_dados', 'modelos_dados', 'modelo_dados_modelo=modelos.modelo_id');
  $sql->adCampo('modelos.modelo_id, modelo_tipo_nome, modelo_protocolo, modelo_assunto, modelo_assunto, modelo_data_assinado, modelos.class_sigilosa');
	$sql->adOnde('modelos_dados_criador	'.($coletivo ? 'IN ('.$Aplic->usuario_lista_grupo.')' : '='.(int)$Aplic->usuario_id));
	$sql->adOnde('modelo_autoridade_assinou IS NOT NULL');
	if ($modelo_tipo_id) $sql->adOnde('modelo_tipo='.(int)$modelo_tipo_id);
	$sql->adGrupo('modelos.modelo_id');
...
elseif($item_menu=='elaboracao'){
  ...
  $sql->esqUnir('modelos_tipo','modelos_tipo','modelo_tipo = modelo_tipo_id');
  $sql->esqUnir('modelos_dados', 'modelos_dados', 'modelo_dados_modelo=modelos.modelo_id');
	$sql->adCampo('modelos.modelo_id, MAX(modelos_dados.modelo_dados_data) AS modelo_dados_data, modelo_tipo_nome, modelo_assunto, modelo_assunto, modelo_protocolo, modelo_data_assinado, modelos.class_sigilosa, modelo_criador_original');
	$sql->adOnde('modelos_dados_criador	'.($coletivo ? 'IN ('.$Aplic->usuario_lista_grupo.')' : '='.(int)$Aplic->usuario_id));
	$sql->adOnde('modelo_versao_aprovada IS NULL');
	if ($modelo_tipo_id) $sql->adOnde('modelo_tipo='.(int)$modelo_tipo_id);
	$sql->adGrupo('modelos.modelo_id');
...
if ($item_menu=='entrada'|| $item_menu=='pendente'||$item_menu=='arquivado' || $item_menu=='enviado'){
	...
	else if ($campo_ordenar=='protocolo')$sql->adOrdem('modelo_protocolo '.$ordem.', modelos.modelo_id DESC, modelo_usuario.status ASC');
	...
	echo '<td>'.dica('Ordenar pelo Nº do Processo','Clique para ordenar pelo número do processo.<br><br>A cada clique será alterada a ordem, entre crescente e decrescente.').'<a href="javascript:void(0);" onclick="javascript:ordenar(\'protocolo\');">'.($campo_ordenar=='protocolo' ? imagem('icones/'.$seta[$sentido]) : '').'<b>Nº Processo</b></a>'.dicaF().'</td>';
	echo '<td>'.dica('Acesso ao histórico','Clique para acessar o histórico.').'<a href="javascript:void(0);" onclick="javascript:ordenar(\'protocolo\');">'.($campo_ordenar=='protocolo' ? imagem('icones/'.$seta[$sentido]) : '').'<b>H</b></a>'.dicaF().'</td>';
	...
	for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
		...
		if (isset($rs_linha['modelo_id']) && $rs_linha['modelo_id']){
		  ...
		  foreach((array)$sql_resultadosc as $rs_anexo){
			...
			echo '<td nowrap="nowrap">'.$rs_linha['modelo_protocolo'].'</td>';
			echo '<td align="center"><a href="javascript:void(0);" onclick="window.open(\'?m=email&a=modelo_exibe_informacoes&modelo_id='.$rs_linha['modelo_id'].($rs_linha ? '&dialogo=1' : '').'\');">'.imagem('icones/ata.gif').dicaF().'</a></td>';								
...
elseif (!(($anexar_documento || $referenciar_documento) && !$pesquisar)){
	...
	echo '<td>'.dica('Acesso rápido','Clique para acessar o processo em outra aba.').'<a href="javascript:void(0);" onclick="javascript:ordenar(\'protocolo\');">'.($campo_ordenar=='protocolo' ? imagem('icones/'.$seta[$sentido]) : '').'<b>Ar</b></a>'.dicaF().'</td>';
	...
	for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
		...
		if(isset($rs['modelo_id']) && $rs['modelo_id']){
			...
			$rs_linha=$resultados[$i];
			if (isset($rs_linha['modelo_id']) && $rs_linha['modelo_id']){
			echo '<td align="center"><a href="javascript:void(0);" onclick="window.open(\'?m=email&a=modelo_acesso&modelo_id='.$rs_linha['modelo_id'].'\');">'.imagem('icones/aviso_p.gif').dicaF().'</a></td>';								
			}
...
function setAgenda(chave, valor){
	...
	function popPDCLItem() {
		if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["pdcl_item"])?>', 1000, 700, 'm=pdcl&a=pdcl_item_lista&dialogo=1&selecao=2&chamarVolta=setPDCLItem&tabela=pdcl_item&cia_id='+document.getElementById('cia_id').value, window.setPDCLItem, window);
		else window.open('./index.php?m=publico&a=selecionar_multiplo&dialogo=1&chamar_volta=setPDCLItem&tabela=pdcl_item&cia_id='+document.getElementById('cia_id').value, '<?php echo ucfirst($config["pdcl_item"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
		}

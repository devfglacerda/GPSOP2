...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome', 'Preencha neste campo um nome para identificação d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Nome:'.dicaF().'</td><td align="left"><input type="text" class="texto" name="instrumento_nome" style="width:400px" value="'.(isset($obj->instrumento_nome) ? $obj->instrumento_nome : '').'"></td></tr>';
echo '<tr><td align=right style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', 'Selecione '.$config['genero_organizacao'].' '.$config['organizacao'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td><div id="combo_cia">'.selecionar_om($cia_id, 'instrumento_cia', 'class=texto size=1 style="width:400px;" onchange="javascript:mudar_om();"').'</div></td></tr>';
if ($Aplic->profissional) {
	$saida_cias='';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Número', 'Preencha neste campo o número de identificação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Número:'.dicaF().'</td><td align="left"><input type="text" class="texto" name="instrumento_numero" style="width:400px" value="'.(isset($obj->instrumento_numero) ? $obj->instrumento_numero : '').'"></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ano', 'Preencha neste campo o ano d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Ano:'.dicaF().'</td><td align="left"><input type="text" class="texto" name="instrumento_ano" style="width:400px" value="'.(isset($obj->instrumento_ano) ? $obj->instrumento_ano : date('Y')).'"></td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Tipo', 'Selecione qual o tipo de instrumento.').'Tipo:'.dicaF().'</td><td align="left" >'.selecionaVetor(getSisValor('TipoInstrumento','','','',true,0), 'instrumento_tipo', 'class="texto" style="width:400px"', (isset($obj->instrumento_tipo) ? $obj->instrumento_tipo : '')).'</td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Objeto', 'Preencha neste campo o objeto d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Objeto:'.dicaF().'</td><td><textarea data-gpweb-cmp="ckeditor" rows="3" name="instrumento_objeto" id="instrumento_objeto">'.(isset($obj->instrumento_objeto) ? $obj->instrumento_objeto : '').'</textarea></td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Justificativa', 'Preencha neste campo a justificativa d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Justificativa:'.dicaF().'</td><td><textarea data-gpweb-cmp="ckeditor" rows="3" name="instrumento_justificativa" id="instrumento_justificativa">'.(isset($obj->instrumento_justificativa) ? $obj->instrumento_justificativa : '').'</textarea></td></tr>';
...
if ($Aplic->profissional){
	...
	$sql->limpar();
	if (count($indicadores)>1) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores relacionados mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'instrumento_principal_indicador', 'class="texto" style="width:400px;"', $obj->instrumento_principal_indicador).'</td></tr>';
	}
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Recursos', 'Quais '.strtolower($config['recursos']).' estão envolvid'.$config['genero_recurso'].'s.').ucfirst($config['recursos']).':'.dicaF().'</td><td><table cellpadding=0 cellspacing=0><tr><td style="width:400px;"><div id="combo_recursos">'.$saida_recursos.'</div></td><td>'.botao_icone('recursos_p.gif','Selecionar', 'selecionar '.$config['recursos'].'.','popRecursos()').'</td></tr></table></td></tr>';
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'instrumento_moeda', 'class=texto size=1', ($obj->instrumento_moeda ? $obj->instrumento_moeda : 1)).'</td></tr>';
	}	
...
function popPerspectiva() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["perspectiva"])?>', 1000, 700, 'm=praticas&a=perspectiva_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setPerspectiva&tabela=perspectivas&cia_id='+document.getElementById('instrumento_cia').value, window.setPerspectiva, window);
...
function popTema() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["tema"])?>', 1000, 700, 'm=praticas&a=tema_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setTema&tabela=tema&cia_id='+document.getElementById('instrumento_cia').value, window.setTema, window);
...
function popObjetivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["objetivo"])?>', 1000, 700, 'm=praticas&a=obj_estrategico_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('instrumento_cia').value, window.setObjetivo, window);
...
function popFator() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["fator"])?>', 1000, 700, 'm=praticas&a=fator_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setFator&tabela=fator&cia_id='+document.getElementById('instrumento_cia').value, window.setFator, window);
...
function popEstrategia() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["iniciativa"])?>', 1000, 700, 'm=praticas&a=estrategia_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setEstrategia&tabela=estrategias&cia_id='+document.getElementById('instrumento_cia').value, window.setEstrategia, window);
...
function popMeta() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["meta"])?>', 1000, 700, 'm=praticas&a=meta_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setMeta&tabela=metas&cia_id='+document.getElementById('instrumento_cia').value, window.setMeta, window);
...
function popPDCLItem() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["pdcl_item"])?>', 1000, 700, 'm=pdcl&a=pdcl_item_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setPDCLItem&tabela=pdcl_item&cia_id='+document.getElementById('instrumento_cia').value, window.setPDCLItem, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setPDCLItem&tabela=pdcl_item&cia_id='+document.getElementById('instrumento_cia').value, '<?php echo ucfirst($config["pdcl_item"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}

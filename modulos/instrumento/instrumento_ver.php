...
if (!$dialogo){	
	...
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Nov'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']), 'Criar um nov'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Nov'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=instrumento&a=instrumento_editar\");");
...
elseif ($dialogo) echo '<table cellpadding=0 cellspacing=1 width="750"><tr><td align=center><h1>'.ucfirst($config['instrumento']).'</h1></td></tr></table>';
...
if ($obj->instrumento_cia) echo '<tr><td align=right style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_cia($obj->instrumento_cia).'</td></tr>';
...
if ($obj->instrumento_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_secao($obj->instrumento_dept).'</td></tr>';
...
if ($saida_depts) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamentos']).' Envolvid'.$config['genero_dept'].'s', 'Quais '.strtolower($config['departamentos']).' estão envolvid'.$config['genero_dept'].'s com '.($config['genero_instrumento']=='o' ? 'este' : 'esta').' '.$config['instrumento'].'.').ucfirst($config['departamentos']).' envolvid'.$config['genero_dept'].'s:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$saida_depts.'</td></tr>';



echo '<tr><td align="right" style="white-space: nowrap">'.dica('Número', 'O número de identificação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Número:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_numero.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ano', 'O ano d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Ano:'.dicaF().'</td><td align="left" class="realce">',$obj->instrumento_ano.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Tipo', 'O tipo de instrumento.').'Tipo:'.dicaF().'</td><td align="left" class="realce">'.getSisValorCampo('TipoInstrumento', $obj->instrumento_tipo).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Objeto', 'O objeto de instrumento.').'Objeto:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_objeto.'</td></tr>';
if ($obj->instrumento_justificativa) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Justificativa', 'A justificativa de instrumento.').'Justificativa:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_justificativa.'</td></tr>';



if ($Aplic->usuario_id==$obj->instrumento_responsavel || $editar){
	$SituacaoInstrumento = getSisValor('SituacaoInstrumento');
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Percentual realizado', 'Indique o porcentual d'.$config['genero_instrumento'].' '.$config['instrumento'].' já completad'.$config['genero_instrumento'].'.').'Realizad'.$config['genero_instrumento'].': '.dicaF().'</td><td align="left" class="realce">'.selecionaVetor($percentual, 'instrumento_porcentagem', 'size="1" class="texto" onchange="mudar_percentagem('. $obj->instrumento_id.', this.value);"', (int)$obj->instrumento_porcentagem).'</td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Situação', 'A situação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Situação:'.dicaF().'</td><td align=left id="status_'.$obj->instrumento_id.'" '.(isset($cor[$obj->instrumento_situacao]) ? 'style="white-space: nowrap; background-color:#'.$cor[$obj->instrumento_situacao].';"' : 'style="white-space: nowrap;"').'>'.selecionaVetor($SituacaoInstrumento, 'instrumento_situacao', 'size="1" class="texto" onchange="mudar_status('. $obj->instrumento_id.', this.value);"', $obj->instrumento_situacao).'</td></tr>';
	}
else {
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Percentual realizado', 'Indique o porcentual d'.$config['genero_instrumento'].' '.$config['instrumento'].' já completad'.$config['genero_instrumento'].'.').'Realizad'.$config['genero_instrumento'].': '.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_porcentagem.'%</td></tr>';
	if ($obj->instrumento_situacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Situação', 'A situação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Situação:'.dicaF().'</td><td align="left" '.(isset($cor[$obj->instrumento_situacao]) ? 'style="white-space: nowrap; background-color:#'.$cor[$obj->instrumento_situacao].';"' : 'style="white-space: nowrap;"').'>'.getSisValorCampo('SituacaoInstrumento', $obj->instrumento_situacao).'</td></tr>';
	}
...
if ($obj->instrumento_licitacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Licitação', 'O tipo de licitação para '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Licitação:'.dicaF().'</td><td align="left" class="realce">'.getSisValorCampo('ModalidadeLicitacao',$obj->instrumento_licitacao).'</td></tr>';
if ($obj->instrumento_edital_nr) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Número do Edital', 'O número do edital da licitação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Número do Edital:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_edital_nr.'</td></tr>';
if ($obj->instrumento_processo) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Número do Processo', 'O número do processo d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Número do Processo:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_processo.'</td></tr>';
if ($obj->instrumento_entidade) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Entidade', 'A entidade com a qual foi celebrado '.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Entidade:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_entidade.'</td></tr>';
if ($obj->instrumento_entidade_cnpj) echo '<tr><td align="right" style="white-space: nowrap">'.dica('CNPJ da Entidade', 'O número do CNPJ da entidade com a qual foi celebrado '.$config['genero_instrumento'].' '.$config['instrumento'].'.').'CNPJ da entidade:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_entidade_cnpj.'</td></tr>';



if ($obj->instrumento_data_celebracao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data de Celebração', 'Data em que '.$config['genero_instrumento'].' '.$config['instrumento'].' foi celebrad'.$config['genero_instrumento'].'.').'Data de celebração:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.retorna_data($obj->instrumento_data_celebracao, false).'</td></tr>';
if ($obj->instrumento_data_publicacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data de Publicação', 'Data em que '.$config['genero_instrumento'].' '.$config['instrumento'].' foi publicad'.$config['genero_instrumento'].'.').'Data de publicação:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.retorna_data($obj->instrumento_data_publicacao, false).'</td></tr>';
if ($obj->instrumento_data_inicio) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data de Início', 'Data de início d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Data de início:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.retorna_data($obj->instrumento_data_inicio, false).'</td></tr>';
if ($obj->instrumento_data_termino) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data de Término', 'Data de término d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Data de término:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.retorna_data($obj->instrumento_data_termino, false).'</td></tr>';
if ($obj->instrumento_valor) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Valor', 'Insira o valor d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Valor:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$config['simbolo_moeda'].'&nbsp;'.number_format($obj->instrumento_valor, 2, ',', '.').'</td></tr>';
if ($obj->instrumento_valor_contrapartida) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Contrapartida', 'Insira o valor da contrapartida d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].', se for o caso.').'Contrapartida:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$config['simbolo_moeda'].'&nbsp;'.number_format($obj->instrumento_valor_contrapartida, 2, ',', '.').'</td></tr>';

if ($obj->instrumento_principal_indicador) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'Dentre os indicadores relacionados, o mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->instrumento_principal_indicador).'</td></tr>';
...
if ($obj->instrumento_supervisor) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['supervisor']), ucfirst($config['genero_supervisor']).' '.$config['supervisor'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['supervisor']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->instrumento_supervisor, '','','esquerda').'</td></tr>';

if ($obj->instrumento_supervisor_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado pel'.$config['genero_supervisor'].' '.ucfirst($config['supervisor']), ucfirst($config['genero_supervisor']).' '.$config['supervisor'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].' aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_supervisor'].' '.$config['supervisor'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->instrumento_supervisor_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->instrumento_supervisor_data).'</td></tr>';
if ($obj->instrumento_supervisor_obs) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observação d'.$config['genero_supervisor'].' '.ucfirst($config['supervisor']), 'A observação redigida pel'.$config['genero_supervisor'].' '.$config['supervisor'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Observação d'.$config['genero_supervisor'].' '.$config['supervisor'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_supervisor_obs.'</td></tr>';

if ($obj->instrumento_autoridade) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['autoridade']), ucfirst($config['genero_autoridade']).' '.$config['autoridade'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['autoridade']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->instrumento_autoridade, '','','esquerda').'</td></tr>';
if ($obj->instrumento_autoridade_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado pel'.$config['genero_autoridade'].' '.ucfirst($config['autoridade']), ucfirst($config['genero_autoridade']).' '.$config['autoridade'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].' aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_autoridade'].' '.$config['autoridade'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->instrumento_autoridade_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->instrumento_autoridade_data).'</td></tr>';
if ($obj->instrumento_autoridade_obs) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observação d'.$config['genero_autoridade'].' '.ucfirst($config['autoridade']), 'A observação redigida pel'.$config['genero_autoridade'].' '.$config['autoridade'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Observação d'.$config['genero_autoridade'].' '.$config['autoridade'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_autoridade_obs.'</td></tr>';

if ($obj->instrumento_cliente) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['cliente']), ucfirst($config['genero_cliente']).' '.$config['cliente'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['cliente']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->instrumento_cliente, '','','esquerda').'</td></tr>';
if ($obj->instrumento_cliente_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado pel'.$config['genero_cliente'].' '.ucfirst($config['cliente']), ucfirst($config['genero_cliente']).' '.$config['cliente'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].' aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_cliente'].' '.$config['cliente'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->instrumento_cliente_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->instrumento_cliente_data).'</td></tr>';
if ($obj->instrumento_cliente_obs) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observação d'.$config['genero_cliente'].' '.ucfirst($config['cliente']), 'A observação redigida pel'.$config['genero_cliente'].' '.$config['cliente'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Observação d'.$config['genero_cliente'].' '.$config['cliente'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_cliente_obs.'</td></tr>';
...
if($saida_quem)echo '<tr><td align="right" style="white-space: nowrap">'.dica('Designado', 'Quais '.$config['usuarios'].' estão designados para '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Designado:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$saida_quem.'</td></tr>';
...
if($saida_quem)echo '<tr><td align="right" style="white-space: nowrap">'.dica('Contatos', 'Quais são os contatos d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Contato:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$saida_quem.'</td></tr>';
...
if ($saida_recurso) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Recurso', 'Qual recurso está relacionado à '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Recurso:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$saida_recurso.'</td></tr>';
...
if (count($lista)) {
	echo '<tr><td align="right" style="white-space: nowrap" valign="middle">'.dica('Relacionad'.$config['genero_instrumento'], 'A que área '.$config['genero_instrumento'].' '.$config['instrumento'].' está relacionad'.$config['genero_instrumento'].'.').'Relacionad'.$config['genero_instrumento'].':'.dicaF().'</td></td><td class="realce">';	
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	if (isset($moedas[$obj->instrumento_moeda])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'A moeda padrão utilizada na meta.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->instrumento_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado', 'Se '.$config['genero_instrumento'].' '.$config['instrumento'].' se encontra aprovad'.$config['genero_instrumento'].'.').'Aprovad'.$config['genero_instrumento'].':'.dicaF().'</td><td  class="realce" width="100%">'.($obj->instrumento_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';


echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ativo', 'Se '.$config['genero_instrumento'].' '.$config['instrumento'].' se encontra ativo.').'Ativo:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->instrumento_ativo ? 'Sim' : 'Não').'</td></tr>';
...
if (!$dialogo) {
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'me')) {

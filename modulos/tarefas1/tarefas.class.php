 ...
 public function ata_acao( $baseline_id=false, $indicador_id=false){
	...
	public function notificarResponsavelProjeto($nao_eh_novo, $tipo='gerente', $email_texto='') {
		...
		if ($usuario['usuario_id']) {
			if (intval($nao_eh_novo)) 
				
			//$corpo = '<b>'.ucfirst($config['genero_tarefa']).' '.ucfirst($config['tarefa']).' '.$this->tarefa_nome.' foi atualizad'.$config['genero_tarefa'].'.</b><br>';
			...
			//else $corpo = '<b>'.ucfirst($config['genero_tarefa']).' '.ucfirst($config['tarefa']).' '.$this->tarefa_nome.' foi criad'.$config['genero_tarefa'].'.</b><br>';
			 ...
			 $corpo .= '<br>Olá, '.$config[$tipo].' d'.$config['genero_projeto'].' '.$config['projeto'].'!<br>';
			
			
			
			$corpo .= '<br><b>Nome do Projeto:</b> '.link_projeto($this->tarefa_projeto).'<br>';
					
						
			$corpo .= '<br><b>Nome da Tarefa:</b> '.link_tarefa($this->tarefa_id).'';
			...
			if (intval($nao_eh_novo)) $corpo .= '<br><br><b>Atualizador d'.$config['genero_tarefa'].' '.$config['tarefa'].':</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
			 ...
			 $corpo .= '<br><br>(Você está recebendo este e-mail por ser '.$config[$tipo].' d'.$config['genero_projeto'].' '.$config['projeto'].')<br>';
			 
			 $corpo .='<br><a href="'.$config['dominio_site'].'/index.php?m=tarefas&a=ver&tarefa_id='.$this->tarefa_id.'">Clique para acessar a tarefa</a><br>';	
	...
	public static function getTarefasParaPeriodo( $data_inicio, $data_fim, $usuario_id=0, $cia_id = null, $dept_id=null,
    $projeto_id=null,
    $tarefa_id=null,
...
public $log_id = null;
  ...
  public $log_reg_mudanca_realizado = null;
	...
	public function armazenar( $atualizarNulos = false) {
		...
		$sql = new BDConsulta();
		if ($this->log_id) {
			$ret = $sql->atualizarObjeto('log', $this, 'log_id', false);
			$sql->limpar();
			}
		else {
	...
	public function notificar($post=array()){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo == 'excluido') {
					$email->Assunto('Excluído registro de andamento de '.$config['tarefa'], $localidade_tipo_caract);
					$titulo='Excluído registro de andamento de '.$config['tarefa'];
					}
				elseif ($tipo=='atualizado') {
					$email->Assunto('Atualizado registro de andamento de '.$config['tarefa'], $localidade_tipo_caract);
					$titulo='Atualizado registro de andamento de '.$config['tarefa'];
					}
				else {
					$email->Assunto('Inserido registro de andamento de '.$config['tarefa'], $localidade_tipo_caract);
					$titulo='Inserido registro de andamento de '.$config['tarefa'];
					}
				if ($tipo=='atualizado') $corpo = 'Atualizado registro de andamento d'.$config['genero_tarefa'].' '.$config['tarefa'].': '.$tarefa_nome.'<br>';
				elseif ($tipo=='excluido') $corpo = 'Excluído registro de andamento d'.$config['genero_tarefa'].' '.$config['tarefa'].': '.$tarefa_nome.'<br>';
				else $corpo = 'Inserido registro de andamento d'.$config['genero_tarefa'].' '.$config['tarefa'].': '.$tarefa_nome.'<br>';


				if ($tipo=='excluido') $corpo .= '<br><br><b>Responsável pela exclusão do registro de andamento:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
				elseif ($tipo=='atualizado') $corpo .= '<br><br><b>Responsável pela edição do registro de andamento:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
				else $corpo .= '<br><b>Criador do registro de andamento:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;

				
				
				

						$corpo.='<table border="1"><br><br><b>Informações sobre '.($config['genero_tarefa']=='o' ? 'este' : 'esta').' '.$config['tarefa'].'</b><br><br>';
						...
						if ($obj->tarefa_dono) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Responsável:</b></td><td>'.nome_usuario($obj->tarefa_dono,'','','esquerda').'</td></tr>';
						if ($obj->tarefa_projeto) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Projeto:</b></td><td>'.nome_projeto($obj->tarefa_projeto,'','','esquerda').'</td></tr>';
						
										
						
						if ($obj->tarefa_status) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Status:</b></td><td width="300">'.getSisValorCampo('StatusTarefa',$obj->tarefa_status).'</td></tr>';
						...
						if ($data_inicio) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Início:</b></td><td width="300">'.$data_inicio->format('%d/%m/%Y %H:%M').'</td></tr>';
						if ($data_fim) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Término:</b></td><td width="300">'.$data_fim->format('%d/%m/%Y %H:%M').'</td></tr>';
						if ($data_inicio && $data_fim && !$obj->tarefa_marco && $obj->tarefa_percentagem > 0 && $obj->tarefa_percentagem < 100){
							...
							$corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Previsão:</b></td><td width="300">'.retorna_data($data).'</td></tr>';
							}
						if ($obj->tarefa_duracao) $corpo.='<tr><td align="right" style="white-space: nowrap" valign="top" style="width:110px;"><b>Duração:</b></td><td width="300">'.number_format((float)$obj->tarefa_duracao/($config['horas_trab_diario'] ? $config['horas_trab_diario'] : 8), 2, ',', '.').' dia'.((float)$obj->tarefa_duracao/($config['horas_trab_diario'] ? $config['horas_trab_diario'] : 8) >= 2 ? 's' : '').'</td></tr>';
						if ($obj->tarefa_tipo) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Tipo:</b></td><td width="300">'.getSisValorCampo('TipoTarefa',$obj->tarefa_tipo).'</td></tr>';
						$unidade= getSisValor('TipoUnidade');
						if ($obj->tarefa_adquirido!=0) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Quantidade adquirida:</b></td><td width="300">'.number_format($obj->tarefa_adquirido, 2, ',', '.').($obj->tarefa_unidade && isset($unidade[$obj->tarefa_unidade]) ? ' '.$unidade[$obj->tarefa_unidade] : '').'</td></tr>';
						if ($obj->tarefa_previsto!=0) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Quantidade prevista:</b></td><td width="300">'.number_format($obj->tarefa_previsto, 2, ',', '.').($obj->tarefa_unidade && isset($unidade[$obj->tarefa_unidade]) ? ' '.$unidade[$obj->tarefa_unidade] : '').'</td></tr>';
						if ($obj->tarefa_realizado!=0) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Quantidade realizada:</b></td><td width="300">'.number_format($obj->tarefa_realizado, 2, ',', '.').($obj->tarefa_unidade && isset($unidade[$obj->tarefa_unidade]) ? ' '.$unidade[$obj->tarefa_unidade] : '').'</td></tr>';
						if ($obj->tarefa_url_relacionada) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;"><b>Endereço URL:</b></td><td width="300"><a href="'.$obj->tarefa_url_relacionada.'" target="tarefa'.$post['log_tarefa'].'">'.$obj->tarefa_url_relacionada.'</a></td></tr>';
						
						if ($obj->tarefa_descricao)	$corpo.='<tr><td align="right" style="white-space: nowrap" align="left" width="80"><b>O Que:</b></td><td>'.$obj->tarefa_descricao.'</td></tr>';
						if ($obj->tarefa_porque)	$corpo.='<tr><td align="right" style="white-space: nowrap"><b>Por Que:</b></td><td>'.$obj->tarefa_porque.'</td></tr>';
						if ($obj->tarefa_como)	$corpo.='<tr><td align="right" style="white-space: nowrap"><b>Como:</b></td><td>'.$obj->tarefa_como.'</td></tr>';
						if ($obj->tarefa_onde)	$corpo.='<tr><td align="right" style="white-space: nowrap"><b>Onde:</b></td><td>'.$obj->tarefa_onde.'</td></tr>';
						
						//if ($this->log_nome)	$corpo.='<tr><td align="right" style="white-space: nowrap"><b>Sumário:</b></td><td>'.$this->log_nome.'</td></tr>';
						if ($this->log_descricao)	$corpo.='<tr><td align="right" style="white-space: nowrap"><b>Registro efetuado:</b></td><td>'.$this->log_descricao.'</td></tr>';
						...
						$corpo .='Link para acesso da tarefa no '.$config['gpweb'].'.<br><br><a href="'.$config['dominio_site'].'\index.php?m=tarefas&a=ver&tarefa_id='.$post['log_tarefa'].'">Clique neste link para acessar a tarefa</a>';	
						}
...
function mostrarTarefaGrande($arr=array(), $projeto_id=0, $editar=true, $tarefa_superior=0, $baseline_id=false){
		...
		$v->adTabela(($baseline_id ? 'baseline_' : '').'recurso_tarefa','recurso_tarefa');
		$v->adCampo('count(recurso_tarefa_recurso)');
		$v->adOnde('recurso_tarefa_tarefa = '.$arr['tarefa_id']);
		if ($baseline_id)	$v->adOnde('recurso_tarefa.baseline_id='.(int)$baseline_id);
		$qnt = $v->Resultado();
		$v->limpar();
...
function mostrarTarefa($arr, $nivel = 0, $aberta = true, $visao_hoje = false, $esconderAbrirFecharLink = false, $permitirRepitir = false, $baseline_id=false) {
	...
	if ($Aplic->usuario_super_admin || permiteAcessar($arr['tarefa_acesso'], $arr['tarefa_projeto'], $arr['tarefa_id'])){
		...
		echo '<td align="center" style="width: 100px;">'.($arr['tarefa_status'] && isset($status[$arr['tarefa_status']]) ? $status[$arr['tarefa_status']] : '&nbsp;').'</td>';

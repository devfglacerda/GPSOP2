...
if ($tab==1){
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_cnpj&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_cnpj' ? imagem('icones/'.$seta[$ordem]) : '').dica('CNPJ', 'Neste campo fica o CNPJ da pessoa jurídica.').'CNPJ'.dicaF().'</a></th>';
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=social_familia_cnes&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_cnes' ? imagem('icones/'.$seta[$ordem]) : '').dica('CNES', 'Neste campo fica o CNES da pessoa jurídica, se for o caso.').'CNES'.dicaF().'</a></th>';
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=social_familia_inep&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_inep' ? imagem('icones/'.$seta[$ordem]) : '').dica('INEP', 'Neste campo fica o INEP da pessoa jurídica, se for o caso.').'INEP'.dicaF().'</a></th>';
	}
else{
	...
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=social_familia_conjuge&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_conjuge' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cônjuge', 'O cônjugue d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Cônjuge'.dicaF().'</a></th>';
	}	
echo '</tr>';

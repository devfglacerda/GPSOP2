...
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=social_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome do Programa Social', 'Neste campo fica um nome para identificação do programa social.').'Nome'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=social_descricao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_descricao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Descrição do Programa Social', 'Neste campo fica a descrição do programa social.').'Descrição'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=social_usuario&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_usuario' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'O '.$config['usuario'].' responsável pelo programa social.').'Responsável'.dicaF().'</a></th>';
echo '</tr>';
$fp = -1;
$id = 0;
$qnt=0;

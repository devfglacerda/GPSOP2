...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
	...
	if (permiteAcessarInstrumento($linha['instrumento_acesso'], $linha['instrumento_id'])){	
		...
		echo '<tr>';
		echo '<td style="white-space: nowrap" width="16">'.($editar ? dica('Editar instrumento', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o instrumento.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=instrumentos&a=instrumento_editar&instrumento_id='.$linha['instrumento_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		echo '<td style="white-space: nowrap">'.dica($linha['instrumento_nome'], 'Clique para visualizar os detalhes deste instrumento.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=instrumentos&a=instrumento_ver&instrumento_id='.$linha['instrumento_id'].'\');">'.$linha['instrumento_nome'].'</a>'.dicaF().'</td>';
		...
		echo '<td style="white-space: nowrap" align="center">'.number_format($linha['instrumento_valor'], 2, ',', '.').'</td>';
		echo '</tr>';
		}
	}

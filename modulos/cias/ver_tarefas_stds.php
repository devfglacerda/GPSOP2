...
$botoesTitulo = new CBlocoTitulo('', 'logo_stds.png', $m, "$m.$a");
...
if (!($linha = $sql->Lista())) {
	...
	echo '<tr>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=tarefa_projeto&ordem='.($ordem ? '0' : '1').'\');">'. dica('Projeto', 'Clique para ordenar pelos projetos.') .'Projeto'.dicaF().'</a></th>';	
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=tarefa_nome&ordem='.($ordem ? '0' : '1').'\');">'. dica('Tarefa', 'Clique para ordenar os contatos pelas tarefas.') .'Tarefas'.dicaF().'</a></th>';
	
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=dept_nome&ordem='.($ordem ? '0' : '1').'\');">'. dica('Seção', 'Clique para ordenar pelo setor.') .'Setor'.dicaF().'</a></th>';
	
	
	
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=usuario_id&ordem='.($ordem ? '0' : '1').'\');">'. dica('Designado', 'Clique para ordenar pelo Designado.') .'Designado'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=tarefa_percentagem&ordem='.($ordem ? '0' : '1').'\');">'. dica('Porcentagem', 'Clique para ordenar pela porcentagem.') .'Porcentagem'.dicaF().'</a></th>';
	
	
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=tarefa_tipo&ordem='.($ordem ? '0' : '1').'\');">'. dica('Tipo', 'Clique para ordenar pelo tipo.') .'Tipo'.dicaF().'</a></th>';
	
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=tarefa_status&ordem='.($ordem ? '0' : '1').'\');">'. dica('Status', 'Clique para ordenar pelo Status.') .'Status'.dicaF().'</a></th>';
	
	
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=tarefa_prioridade&ordem='.($ordem ? '0' : '1').'\');">'. dica('Prioridade', 'Clique para ordenar pela prioridade.') .'Prioridade'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=tarefa_inicio&ordem='.($ordem ? '0' : '1').'\');">'. dica('Data Inicial', 'Clique para ordenar pela data inicia.') .'Data Inicial'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=ver_stds&cia_id='.(int)$cia_id.'&tab='.$tab.'&ordenar=tarefa_fim&ordem='.($ordem ? '0' : '1').'\');">'. dica('Data Final', 'Clique para ordenar pela data final.') .'Data Final'.dicaF().'</a></th>';

...
global $mostrarCaixachecarEditar, $tarefas, $prioridades, $projeto_id, $tarefa_id, $dialogo, $Aplic, $cia_id, $obj, $config, $tab;
...
$mostrarNomeTarefa=nome_projeto($tarefa_id);
$qnt=0;
...
$botoesTitulo = new CBlocoTitulo('Tarefas Paralisadas', 'arquivo.png', $m, "$m.$a");
...
$sql->adCampo('projeto_nome, usuario_id, projeto_setor, tarefa_projeto, tarefa_nome, tarefa_percentagem, tarefa_inicio, tarefa_fim, tarefa_prioridade');		
$sql->adOnde('projeto_ativo = 1');
$sql->adOnde('projeto_template = 0');
$sql->adOnde('tarefa_status = 6');
...
if (!($linha = $sql->Lista())) {
	...
	echo '<tr>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=depts&a=ver&dept_id='.(int)$dept_id.'&tab='.$tab.'&ordenar=tarefa_projeto&ordem='.($ordem ? '0' : '1').'\');">'. dica('Projeto', 'Clique para ordenar pelos projetos.') .'Projeto'.dicaF().'</a></th>';	
		
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=depts&a=ver&dept_id='.(int)$dept_id.'&tab='.$tab.'&ordenar=tarefa_nome&ordem='.($ordem ? '0' : '1').'\');">'. dica('Tarefa', 'Clique para ordenar os contatos pelas tarefas.') .'Tarefa'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=depts&a=ver&dept_id='.(int)$dept_id.'&tab='.$tab.'&ordenar=tarefa_prioridade&ordem='.($ordem ? '0' : '1').'\');">'. dica('Prioridade', 'Clique para ordenar pela prioridade.') .'P'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=depts&a=ver&dept_id='.(int)$dept_id.'&tab='.$tab.'&ordenar=usuario_id&ordem='.($ordem ? '0' : '1').'\');">'. dica('Designado', 'Clique para ordenar pelo Designado.') .'Designado'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=depts&a=ver&dept_id='.(int)$dept_id.'&tab='.$tab.'&ordenar=tarefa_percentagem&ordem='.($ordem ? '0' : '1').'\');">'. dica('Porcentagem', 'Clique para ordenar pela porcentagem.') .'Porcentagem'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=depts&a=ver&dept_id='.(int)$dept_id.'&tab='.$tab.'&ordenar=projeto_setor&ordem='.($ordem ? '0' : '1').'\');">'. dica('Secretaria', 'Clique para ordenar pela secretaria.') .'Secretaria'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=depts&a=ver&dept_id='.(int)$dept_id.'&tab='.$tab.'&ordenar=tarefa_inicio&ordem='.($ordem ? '0' : '1').'\');">'. dica('Data Inicial', 'Clique para ordenar pela data inicia.') .'Data Inicial'.dicaF().'</a></th>';
	echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=depts&a=ver&dept_id='.(int)$dept_id.'&tab='.$tab.'&ordenar=tarefa_fim&ordem='.($ordem ? '0' : '1').'\');">'. dica('Data Final', 'Clique para ordenar pela data final.') .'Data Final'.dicaF().'</a></th>';
	...
	foreach($lista as $linha)  {
		...
		echo '<td align="center">'.($linha['projeto_setor'] && isset($tipo[$linha['projeto_setor']]) ? $tipo[$linha['projeto_setor']] : '&nbsp;').'</td>';
		echo '<td align="center">'.retorna_data($linha['tarefa_inicio']).'</td>';
		echo '<td align="center">'.retorna_data($linha['tarefa_fim']).'</td>

...
class CArquivo extends CAplicObjeto {
	...
	function getNomeProjeto($id_projeto) {
    $sql = new BDConsulta();
    $sql->adTabela('projetos');
    $sql->adCampo('projeto_nome');
    $sql->adOnde('projeto_id =' . $id_projeto);
    $nome_projeto = $sql->Resultado();
    return $nome_projeto['projeto_nome'];
}
    ...
    public function notificar($post = array()) {
        ...
        $sql->adTabela('arquivo');
        $sql->adCampo('arquivo_nome, arquivo_projeto');
        $sql->adOnde('arquivo_id =' . $this->arquivo_id);
        $resultado = $sql->Resultado();
        $nome_arquivo = $resultado['arquivo_nome'];
        $id_projeto = $resultado['arquivo_projeto'];
        $nome_projeto = getNomeProjeto($id_projeto); // Obtém o nome do projeto com base no ID do projeto vinculado ao arquivo
        $sql->limpar();
        ...
        if (isset($post['del']) && $post['del']) {
            $tipo = 'excluido';
        } elseif (isset($post['arquivo_id']) && $post['arquivo_id']) {
            $tipo = 'atualizado';
        } else {
            $tipo = 'incluido';
        }
        ...
        foreach ($usuarios as $usuario) {
            if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])) {
                ...
                if ($tipo == 'atualizado') $corpo = 'Atualizado o arquivo: ' . $nome_arquivo . '<br>';
                elseif ($tipo == 'excluido') $corpo = 'Excluído o arquivo: ' . $nome_arquivo . '<br>';
                else $corpo = 'Inserido o arquivo: ' . $nome_arquivo . '<br>';

                // Adiciona o nome do projeto ao corpo da mensagem
                $corpo .= '<br><b>Projeto:</b> ' . $nome_projeto;

                $corpo .= '<br><br>' . $Aplic->getAplicUrl() . '/aplic.php?acao=arquivo&arquivo_id=' . $this->arquivo_id;
                $email->Mensagem($corpo);
                $email->Para($usuario['contato_email']);
                $email->Envia();
            }
        }
    }

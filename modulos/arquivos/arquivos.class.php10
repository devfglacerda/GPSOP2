...
class CArquivo extends CAplicObjeto {
	...
	public function notificar( $post=array()){
		...
		if (isset($post['arquivo_usuarios']) && $post['arquivo_usuarios'] && isset($post['email_designados'])){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo == 'excluido') $titulo='Arquivo excluído no GPSOP';
				elseif ($tipo=='atualizado') $titulo='Arquivo atualizado no GPSOP';
				else $titulo='Arquivo inserido no GPSOP';

				$email->Assunto($titulo, $localidade_tipo_caract);
				
				$corpo = '<br><a href="javascript:void(0);" onclick="url_passar(0, \'m=arquivos&a=ver&arquivo_id='.$this->arquivo_id.'\');"><b>Clique para acessar o arquivo</b></a>';

				if ($tipo=='atualizado') $corpo = '<b>Atualizado o arquivo:</b> '.$nome.'<br>';
				elseif ($tipo=='excluido') $corpo = '<b>Excluído o arquivo:</b> '.$nome.'<br>';
				else $corpo = '<b>Inserido o arquivo:</b> '.$nome.'<br>';
				
				$corpo .= '<br><b>Descrição:</b>'.$this->arquivo_descricao.'';	
		...
		$sql->limpar();		
			...
			$corpo = '<br><br><b>Nome do Projeto:</b> '.$nome2.'<br>';
				
			$sql = new BDConsulta;
			$sql->adTabela('arquivo');
			$sql->adCampo('arquivo_descricao');
			$nome3 = $sql->Resultado();
			$sql->limpar();	
				
				
				$corpo .= '<br><br><b>Descrição:</b> '.$nome3.'<br>';*/
											
				if ($tipo=='excluido') $corpo .= '<b>Responsável pela exclusão do arquivo:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra.'<br>';
				elseif ($tipo=='atualizado') $corpo .= '<br><br><b>Responsável pela edição do arquivo:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra.'<br>';
				
				
				
				else $corpo .= '<br><br><b>Criador do arquivo:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra.'<br><br>';
...
//				$corpo .= '<br><a href="javascript:void(0);" onclick="url_passar(0, \'m=arquivos&a=ver&arquivo_id='.$this->arquivo_id.'\');"><b>Clique para acessar o arquivo</b></a>';
				...
				$corpo .='<br>Link para acessar o arquivo no '.$config['gpweb'].'.<br><br><a href="'.$config['dominio_site'].'\index.php?m=arquivos&a=saida&arquivo_id='.$this->arquivo_id.'">Clique para marcar saída do arquivo</a>';	

...
class CArquivo extends CAplicObjeto {
    ...
    $nome = $sql->Linha(); // Usar Linha() para obter um array associativo
    $sql->limpar();

    $projeto_nome = $this->getRelacaoArquivo($this->arquivo_id); // Obtém o nome do projeto relacionado

    $usuarios = array_merge(
        $this->fetchUsers('arquivo_usuarios', $post, $config, $sql),
        $this->fetchUsers('email_outro', $post, $config, $sql),
        $this->fetchOwners($post, $sql),
        $this->fetchExtraEmails($post)
    );

    $usado = array();

    // Corrigindo a lógica para determinar o tipo de operação
    if (isset($post['del']) && $post['del']) {
        $tipo = 'exclusão';
    } elseif (isset($post['arquivo_id']) && $post['arquivo_id'] && !$post['del']) {
        $tipo = 'atualização';
    } else {
        $tipo = 'inclusão';
    }
    ...
    foreach ($usuarios as $usuario) {
        if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])) {
            ...
            $this->setResponderPara($email, $Aplic);

            // Ajustar o título e corpo do email conforme o tipo de operação
            if ($tipo == 'exclusão') {	
                $titulo = 'Arquivo excluído';
                $corpo = 'Excluído o arquivo: ' . $nome['arquivo_nome'] . '<br>';
            } elseif ($tipo == 'atualização') {
                $titulo = 'Arquivo atualizado';
                $corpo = '<b>Atualizado o arquivo:</b> ' . $nome['arquivo_nome'] . '<br>';
            } else {
                $titulo = 'Arquivo inserido';
                $corpo = '<b>Inserido o arquivo:</b> ' . $nome['arquivo_nome'] . '<br>';
            }

            if ($projeto_nome) {
                $corpo .= '<b>Projeto:</b> ' . $projeto_nome . '<br>';
            }
            $corpo .= '<br><br><b>Responsável pela ' . $tipo . ' do arquivo:</b> ' . $Aplic->usuario_posto . ' ' . $Aplic->usuario_nomeguerra;

            if ($tipo != 'exclusão') {
                $corpo .= '<br><a href="https://gpsop.sop.ce.gov.br/index.php?m=arquivos&a=saida&arquivo_id=' . $this->arquivo_id . '"><b>Clique para acessar o arquivo</b></a>';
            }

            $email->Assunto($titulo);  // Adiciona o assunto ao email
...
public function getRelacaoArquivo($arquivo_id) {
    $sql = new BDConsulta;

    // Consultar a tabela arquivo_gestao para obter o ID do projeto relacionado ao arquivo
    $sql->adTabela('arquivo_gestao');
    $sql->adCampo('arquivo_gestao_projeto');
    $sql->adOnde('arquivo_gestao_arquivo = ' . intval($arquivo_id));
    $resultado = $sql->Linha(); // Usar Linha() para obter um array associativo
    $sql->limpar();

    if ($resultado) {
        $projeto_id = $resultado['arquivo_gestao_projeto'];

        // Consultar a tabela projetos para obter o nome do projeto
        $sql->adTabela('projetos');
        $sql->adCampo('projeto_nome');
        $sql->adOnde('projeto_id = ' . intval($projeto_id));
        $resultado = $sql->Linha(); // Usar Linha() para obter um array associativo
        $sql->limpar();

        if ($resultado) {
            return $resultado['projeto_nome'];
        }
    }

    return null; // Caso não encontre a relação
}

private function fetchUsers($key, $post, $config, $sql) {
    if (!isset($post[$key]) || !$post[$key]) return array();
    ...
    $sql->adOnde('usuario_id IN (' . $post[$key] . ')');
    $result = $sql->Lista();
    $sql->limpar();
    return $result;
}

private function fetchOwners($post, $sql) {
    if (!isset($post['email_responsavel']) || !$post['email_responsavel']) return array();

    $sql->adTabela('usuarios');
    $sql->esqUnir('contatos', 'contatos', 'contato_id = usuario_contato');
    $sql->esqUnir('arquivo', 'arquivo', 'arquivo.arquivo_dono = usuarios.usuario_id');
    $sql->adCampo('DISTINCT usuarios.usuario_id, contato_nomeguerra AS nome_usuario, contato_email');
    $sql->adOnde('arquivo_id=' . $this->arquivo_id);
    $result = $sql->Lista();
    $sql->limpar();
    return $result;
...
private function fetchExtraEmails($post) {
    if (!isset($post['email_extras']) || !$post['email_extras']) return array();

    $extras = explode(',', $post['email_extras']);
    return array_map(function($email) {
        return array('usuario_id' => 0, 'nome_usuario' => '', 'contato_email' => $email);
    }, $extras);
}

private function setResponderPara($email, $Aplic) {
    if ($this->isEmailValido($Aplic->usuario_email)) {
        $email->ResponderPara($Aplic->usuario_email);
    } elseif ($this->isEmailValido($Aplic->usuario_email2)) {

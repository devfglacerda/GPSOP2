...
class CArquivo extends CAplicObjeto {
    ...
    $projetoNome = $this->getProjetoNome($this->arquivo_id);
...
public function getProjetoNome($arquivo_id) {
    $sql = new BDConsulta;
    $sql->adTabela('arquivo');
    $sql->adCampo('arquivo_projeto');
    $sql->adOnde('arquivo_id = ' . (int)$arquivo_id);
    $resultado = $sql->Resultado();

    if ($resultado) {
        $projeto_id = $resultado['arquivo_projeto'];

        $sql_projeto = new BDConsulta;
        $sql_projeto->adTabela('projetos');
        $sql_projeto->adCampo('projeto_nome');
        $sql_projeto->adOnde('projeto_id = ' . (int)$projeto_id);
        $resultado_projeto = $sql_projeto->Resultado();

        if ($resultado_projeto) {
            return $resultado_projeto['projeto_nome'];
        } else {

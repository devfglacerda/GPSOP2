...
foreach ($usuarios as $usuario) {
    if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])) {
		...
		$projetoNome = $this->getProjetoNome();
        ...
        $corpo_externo = $corpo;
				               ...
				               $corpo_interno .= '<br><br><b>Projeto:</b> ' . $projetoNome;
					   $corpo_externo .= '<br><br><b>Projeto:</b> ' . $projetoNome;
...
public function getProjetoNome() {
    ...
    // Verifica se o arquivo está associado a um projeto
    if (!$this->arquivo_projeto) {
        return $projetoNome;
    }

    // Limpa consultas anteriores e executa nova consulta para obter o nome do projeto
    $this->_consulta->limpar();
    $this->_consulta->adTabela('projetos');
    $this->_consulta->adCampo('projeto_nome');
    $this->_consulta->adOnde('projeto_id = ' . (int)$this->arquivo_projeto);
    $resultadoConsulta = $this->_consulta->exec();

    // Verifica se a consulta foi bem-sucedida
    if ($resultadoConsulta) {
        // Obtém o nome do projeto a partir do resultado da consulta
        if (isset($resultadoConsulta->fields['projeto_nome'])) {
            $projetoNome = $resultadoConsulta->fields['projeto_nome'];
        }
    }

    // Limpa a consulta e retorna o nome do projeto
    $this->_consulta->limpar();
    return $projetoNome;
}

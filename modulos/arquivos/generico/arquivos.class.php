...
// Nova consulta para o nome do projeto
$nome_do_projeto = ''; // Inicializa o nome do projeto como vazio
$sql_projeto = new BDConsulta;
$sql_projeto->adTabela('arquivo AS a, projetos AS p'); // Adiciona as tabelas 'arquivo' e 'projetos' à consulta com aliases
$sql_projeto->adCampo('p.projeto_nome'); // Adiciona o campo 'projeto_nome' à consulta
$sql_projeto->adOnde('a.arquivo_projeto = p.projeto_id'); // Faz a condição de junção entre as tabelas
$sql_projeto->adOnde('a.arquivo_id =' . $this->arquivo_id);
$resultado_projeto = $sql_projeto->Resultado();
if ($resultado_projeto) {
    $nome_do_projeto = $resultado_projeto['projeto_nome'];
}
$sql_projeto->limpar();
    ...
    if (isset($post['email_extras']) && $post['email_extras']) {
        ...
        foreach ($extras as $chave => $valor) {
            $usuarios4[] = array('usuario_id' => 0, 'nome_usuario' => '', 'contato_email' => $valor);
        }
    }
    ...
    foreach ($usuarios as $usuario) {
        if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])) {
            ...
            $corpo = ''; // Inicializa o corpo do email como vazio
            ...
            if ($nome_do_projeto) { // Verifica se há um nome de projeto
                $corpo .= '<br><b>Projeto:</b> ' . $nome_do_projeto . '<br>'; // Adiciona o nome do projeto ao corpo do email
            }
            ...
            if ($tipo != 'excluido') {
                // Link interno para todos os usuários
				$corpo_interno .= '<br><b>Projeto:</b> ' . $nome_do_projeto . '<br>'; // Adiciona o nome do projeto ao corpo do email
                ...
                // Link externo para todos os usuários
				
				$corpo_externo .= '<br><b>Projeto:</b> ' . $nome_do_projeto . '<br>'; // Adiciona o nome do projeto ao corpo do email
                $corpo_externo .= '<br><a href="https://gpsop.sop.ce.gov.br/index.php?m=arquivos&a=ver&arquivo_id=' . $this->arquivo_id . '"><b>Clique para acessar o arquivo</b></a>';
            }

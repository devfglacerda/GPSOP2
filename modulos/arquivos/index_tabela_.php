...
foreach ($arquivos as $linha) {
	...
	/* 	*/
		...
		echo '<td>'.($linha['saida'] ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=arquivos&a=ver_historico_pro&arquivo_id='.$linha['arquivo_id'].'\');">'.imagem('icones/info.gif', 'Histórico', 'Ao clicar neste ícone será possivel ler o histórico de saída do arquivo.').'</a>' : '&nbsp;').'</td>';
...
function ver_historico(arquivo_id){
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Histórico de Retirada', 1024, 800, 'm=arquivos&a=ver_historico_pro&arquivo_id='+arquivo_id, null, window);
	else window.open('./index.php?m=arquivos&a=ver_historico_pro&arquivo_id='+arquivo_id, 'Histórico','left=0,top=0,height=1024,width=800,scrollbars=yes, resizable=yes');
	}

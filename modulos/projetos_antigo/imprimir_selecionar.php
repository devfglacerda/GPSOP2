...
if ($config['anexo_mpog']){
	...
	if ($linha1['demanda_caracteristica_projeto']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=demanda_analise_imprimir&dialogo=1&demanda_id='.$linha1['demanda_id'].'\');">Mensuração do Tamanho d'.$config['genero_projeto'].' '.$config['projeto'].' (MTP)</a></td></tr>';
	if ($linha1['demanda_viabilidade']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=viabilidade_imprimir&dialogo=1&projeto_viabilidade_id='.$linha1['demanda_viabilidade'].'\');">Análise de Viabilidade d'.$config['genero_projeto'].' '.$config['projeto'].' (AVP)</a></td></tr>';
...
function ir_para2(url){
	...
	if (exportar) {
	 	...
	 	window.open('./index.php?m=publico&a=exportar_link&dialogo=1&tipo=endereco&endereco='+document.getElementById('cifrado').value, 'Link', 'height=100,width=900,resizable,scrollbars=yes');
		}
